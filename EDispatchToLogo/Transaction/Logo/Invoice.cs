...
namespace EDispatchToLogo.Transaction.Logo
{
    ...
    {
        public static Model.Transfer.TransferResult Add(UnityApplication pMyApp, Model.Transfer.Invoice pInvoice)
        {
            ...
            UnityObjects.IData lObject = pMyApp.NewDataObject(DataObjectType.doSalesInvoice);

            lObject.New();
            lObject.DataFields.FieldByName("TYPE").Value = 8;
            lObject.DataFields.FieldByName("NUMBER").Value = "~";
            //lObject.DataFields.FieldByName("DOC_NUMBER").Value = "";
            lObject.DataFields.FieldByName("DATE").Value = pInvoice.FicheDate;
            lObject.DataFields.FieldByName("TIME").Value = 270276609;


            if (pInvoice.AcceptInv.Equals(1))
            {
                lObject.DataFields.FieldByName("EINVOICE").Value = 1;
                lObject.DataFields.FieldByName("EDURATION_TYPE").Value = 0;
            }
            else
            {
                lObject.DataFields.FieldByName("EINVOICE").Value = 2;
                lObject.DataFields.FieldByName("EARCHIVEDETR_EARCHIVESTATUS").Value = 0;
                lObject.DataFields.FieldByName("EARCHIVEDETR_INTPAYMENTTYPE").Value = "4";
                lObject.DataFields.FieldByName("EBOOK_DOCTYPE").Value = "99";
            }

            lObject.DataFields.FieldByName("PROFILE_ID").Value = pInvoice.ProfileID;            
            lObject.DataFields.FieldByName("DOC_DATE").Value = lObject.DataFields.FieldByName("DATE").Value;                        
            lObject.DataFields.FieldByName("ARP_CODE").Value = pInvoice.ArpCode;
            lObject.DataFields.FieldByName("NOTES1").Value = pInvoice.Description1;
            lObject.DataFields.FieldByName("NOTES2").Value = pInvoice.Description2;
            lObject.DataFields.FieldByName("SHIPPING_AGENT").Value = pInvoice.ShippingCode;
            ...
            foreach (var line in pInvoice.Lines)
            {
                ILine.AppendLine();
                ...
                ILine[ILine.Count - 1].FieldByName("PRICE").Value = Helper.Common.GetConvertToDouble(line.UnitPrice);
                ILine[ILine.Count - 1].FieldByName("TOTAL").Value = Helper.Common.GetConvertToDouble(line.UnitPrice);
                ...
                ILine[ILine.Count - 1].FieldByName("VAT_BASE").Value = Helper.Common.GetConvertToDouble(line.UnitPrice);
                ILine[ILine.Count - 1].FieldByName("TOTAL_NET").Value = Helper.Common.GetConvertToDouble(line.UnitPrice);
            ...
            //lObject.DataFields.FieldByName("SHIP_DATE").Value = pInvoice.FicheDate;
            //lObject.DataFields.FieldByName("SHIP_TIME").Value = 270276609;
            lObject.DataFields.FieldByName("DOC_DATE").Value = pInvoice.FicheDate;
